...
def _get_pydir(session):
    ...
    if version_info < (3, 13):
        session.error("Only Python >= 3.13 is supported")
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/gentoo/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/gentoo/*")
            ...
            #    "--include=src/saltext/gentoo/*,tests/*"
            # )
        finally:
